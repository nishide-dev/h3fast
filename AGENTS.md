...
Formal quality-set readiness is recorded in `benchmarks/quality/formal-quality-set.json`. The committed record is intentionally `incomplete` and contains no prompt text or media. A local 10-smoke / 50-regression candidate is identified only by the redacted digest attestation in `benchmarks/quality/formal-quality-registry-attestation.json`; its cases remain uncommitted pending rights review. Keep private registry instances and reference assets outside Git; use `h3fast benchmark compile-quality-registry` to produce redacted candidates. Do not commit per-case digest metadata or set `formal_quality_set_ready` or the release gate to passed until the registry contents, rights evidence, metric plans, and named rights/quality approvals have been reviewed and pass `h3fast benchmark check-quality-set`.
