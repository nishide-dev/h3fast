...
4. Preserve the approved Japan-local single-operator H3-use scope; return the territory inventory to `incomplete` before any new operator, country, third-party access, derivative distribution, Output distribution, or service.
...
Territory evidence for H3-related flows is recorded separately in `compliance/territories/initial-runtime.json`. The approved scope is limited to `nishide-dev` using the declared Japan-local machine and storage for local research without third-party H3 Works or Output access. Do not infer new locations from timezone, IP geolocation, runner labels, or repository hosting. Return the record to `incomplete` and review it before any operator, physical country, third-party access, derivative distribution, Output distribution, Hosted Service, or storage-provider change. Source storage, CPU-only CI, and code-only package access are `not-applicable` and are not part of the package release gate.

Formal quality-set readiness is recorded in `benchmarks/quality/formal-quality-set.json`. The committed record is intentionally `incomplete` and contains no prompt text or media. A local 10-smoke / 50-regression candidate is identified only by the redacted digest attestation in `benchmarks/quality/formal-quality-registry-attestation.json`; its cases remain uncommitted pending rights review. Keep private registry instances, private review instances, reference assets, and per-case digests outside Git. Use `h3fast benchmark prepare-quality-review` and `apply-quality-review` for digest-bound local decisions, and `compile-quality-registry` only after the review is complete. Running these commands is not human approval. The six-family evaluation contract is `benchmarks/quality/formal-quality-metric-plan.json`; it remains `draft` until exact implementations, immutable revisions, dependency pins, owners, baseline-envelope budgets, and evidence are reviewed. Do not commit per-case digest metadata or set `formal_quality_set_ready` or the release gate to passed until the registry contents, rights evidence, metric plans, and named rights/quality approvals have been reviewed and pass both metric-plan and formal-set checks.
