...
1. smoke splitから4 case以上を選び、baselineとcandidateで各1反復生成する。
2. per-case生成物のSHA-256を照合する。
3. 全caseが一致した場合、当該最適化はbit-exactとして採用判定できる。exact artifact gate、protocol差分test、実効setting一致、E2E/stage/memory測定は従来どおり要求する。
4. 1 caseでもdigest不一致がある場合、Tier 1判定は成立しない。Tier 2へescalateする。
