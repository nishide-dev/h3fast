...
| Formal quality-set record | H3Fast独自metadata | 10/50件のcase identity、coverage、rights/metric approval契約。prompt本文、reference media、生成物なし | Apache-2.0 | Public候補 | `incomplete`。review済みcase metadata、metrics、formal set approvalは未登録 |
...
| Private quality registry | H3Fast所有または権利review対象のprompt/reference input | prompt本文、local asset path、rights evidenceを含み得る。H3 Materials/Outputは含めない | 入力ごとの権利に従う | repository外・access制御 | 10 smoke / 50 regression candidateとsynthetic referenceをlocal生成済み。承認証跡のimmutable URL適用待ち、Git管理禁止 |
| Private quality review | H3Fast独自のlocal review decision | registry/content digest、per-case digest、rights/selection decision、evidenceを含む。prompt/media/pathなし | Apache-2.0候補 | repository外・access制御 | 60件とselection判断を承認済み。immutable evidence適用前後ともGit/CI artifact化禁止 |
